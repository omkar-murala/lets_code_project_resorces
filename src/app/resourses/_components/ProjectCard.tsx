...
function ProjectCard() {
  const { data: datas, loading } = useFetch("getprogramproject", "getprogramproject");
      ...
      <div className="p-6 shadow-lg rounded-lg bg-slate-800 mb-10 mt-4">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {datas?.slice(0, itemsToShow).map((topic: any) => (
              <Link key={topic.id} href={`projects/${topic.link}`}>
