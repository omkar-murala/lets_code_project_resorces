...
function LearningCard() {
  const { data: datas, loading } = useFetch("roadmaps", "roadmaps");
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {datas?.slice(0, itemsToShow).map((topic: any, index: number) => (
              <Link key={index} href={`resouces/${topic.link}`}>
                <div className="flex flex-col h-[150px] items-center bg-gradient-to-r from-green-300 to-green-400 text-white p-6 rounded-lg shadow-lg hover:from-green-500 hover:to-green-500 transition transform hover:-translate-y-1">
                  <Image
                    src={topic.imageUrl}
                    alt={topic.name}
                    width={64}
